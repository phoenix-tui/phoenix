...
func TestFlex_AlignMethods(t *testing.T) {
	tests := []struct {
		name     string
		setup    func() *Flex
		expected value2.AlignItems
	}{
		{
			name:     "AlignStretch",
			setup:    func() *Flex { return Row().AlignStretch() },
			expected: value2.AlignItemsStretch,
		},
		{
			name:     "AlignStart",
			setup:    func() *Flex { return Row().AlignStart() },
			expected: value2.AlignItemsStart,
		},
		{
			name:     "AlignEnd",
			setup:    func() *Flex { return Row().AlignEnd() },
			expected: value2.AlignItemsEnd,
		},
		{
			name:     "AlignCenter",
			setup:    func() *Flex { return Row().AlignCenter() },
			expected: value2.AlignItemsCenter,
		},
	}
