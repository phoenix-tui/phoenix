...
func (ls *LayoutService) Layout(box *model.Box, parentSize value.Size) value.Position {
	...
	return value.NewPosition(xOffset, yOffset)
}
...
func (ls *LayoutService) LayoutNode(node *model.Node, parentSize value.Size) *model.Node {
	...
	result := model.NewNode(rootBox).SetPosition(rootPosition)
	...
	for _, child := range children {
		...
		childParentSize := value.NewSizeExact(
			parentSize.Width(),
