...
// TestNewMeasureService tests constructor validation.
func TestNewMeasureService(t *testing.T) {
	t.Run("valid unicode service", func(t *testing.T) {
		us := coreService.NewUnicodeService()
		ms := NewMeasureService(us)
		assert.NotNil(t, ms)
	})

	t.Run("nil unicode service panics", func(t *testing.T) {
		assert.Panics(t, func() {
			NewMeasureService(nil)
		}, "should panic with nil unicode service")
	})
}
...
func TestMeasure_ComplexBox(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			hasBorder:      true,
			margin:         value.NewSpacing(2, 3, 2, 3),
...
func TestMeasure_WithSizeConstraints(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:            "exact size",
			content:         "Hello",
			sizeConstraints: value.NewSizeExact(10, 5),
			expectedWidth:   10,
			expectedHeight:  5,
		},
		{
			...
			content:         "Hi",
			sizeConstraints: value.NewSize(-1, -1, 10, -1, -1, -1),
		...
		},
		{
			...
			content:         strings.Repeat("A", 100),
			sizeConstraints: value.NewSize(-1, -1, -1, 50, -1, -1),
		...
		},
		{
			...
			content:         "Single line",
			sizeConstraints: value.NewSize(-1, -1, -1, -1, 5, -1),
			expectedWidth:   11,
		...
		},
		{
			...
			content:         "A\nB\nC\nD\nE\nF\nG\nH\nI\nJ",
			sizeConstraints: value.NewSize(-1, -1, -1, -1, -1, 5),
			expectedWidth:   1,
		...
		},
		{
			...
			content:         "Test",
			sizeConstraints: value.NewSize(-1, -1, 10, 20, 3, 8),
		...
		},
		{
			name:            "unconstrained",
			content:         "Hello World",
			sizeConstraints: value.NewSizeUnconstrained(),
			expectedWidth:   11,
			expectedHeight:  1,
		},
	}
...
func TestMeasure_EdgeCases(t *testing.T) {
	...
	t.Run("very large padding", func(t *testing.T) {
		box := model.NewBox("X").
			WithPadding(value.NewSpacingAll(100))
