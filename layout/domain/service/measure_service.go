...
func NewMeasureService(us *coreService.UnicodeService) *MeasureService {
	if us == nil {
		panic("measure_service: unicode service cannot be nil")
	}
	return &MeasureService{unicodeService: us}
}
...
func (ms *MeasureService) measureContent(content string) (width, height int) {
	...
	for _, line := range lines {
		lineWidth := ms.unicodeService.StringWidth(line)
