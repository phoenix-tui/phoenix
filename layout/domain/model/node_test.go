...
func TestNode_FluentAPI(t *testing.T) {
	tree := NewNode(NewBox("Root")).
		...
		SetPosition(value.NewPosition(10, 5))
...
func TestIntegration_BoxAndNode(t *testing.T) {
	...
	node := NewNode(box).
		AddChildren(
			NewNode(NewBox("Child 1").WithPadding(value.NewSpacingAll(1))),
