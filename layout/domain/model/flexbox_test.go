...
func TestNewFlexContainer(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Row direction",
			direction: value.FlexDirectionRow,
			wantPanic: false,
		},
		{
			name:      "Column direction",
			direction: value.FlexDirectionColumn,
			wantPanic: false,
		},
		{
			name:      "Invalid direction panics",
			direction: value.FlexDirection(99),
			wantPanic: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(_ *testing.T) {
			...
			if container.JustifyContent() != value.JustifyContentStart {
				t.Errorf("JustifyContent() = %v, want %v", container.JustifyContent(), value.JustifyContentStart)
			}
			if container.AlignItems() != value.AlignItemsStretch {
				t.Errorf("AlignItems() = %v, want %v", container.AlignItems(), value.AlignItemsStretch)
			}
			if container.Gap() != 0 {
...
func TestFlexContainer_WithDirection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(_ *testing.T) {
			...
			// Check immutability
			if container.Direction() != value.FlexDirectionRow {
...
func TestFlexContainer_WithJustifyContent(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Set to start",
			justify:   value.JustifyContentStart,
			wantPanic: false,
		},
		{
			name:      "Set to end",
			justify:   value.JustifyContentEnd,
			wantPanic: false,
		},
		{
			name:      "Set to center",
			justify:   value.JustifyContentCenter,
			wantPanic: false,
		},
		{
			name:      "Set to space-between",
			justify:   value.JustifyContentSpaceBetween,
			wantPanic: false,
		},
		{
			name:      "Invalid justify panics",
			justify:   value.JustifyContent(99),
			wantPanic: true,
		},
	}
...
func TestFlexContainer_WithAlignItems(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Set to stretch",
			align:     value.AlignItemsStretch,
			wantPanic: false,
		},
		{
			name:      "Set to start",
			align:     value.AlignItemsStart,
			wantPanic: false,
		},
		{
			name:      "Set to end",
			align:     value.AlignItemsEnd,
			wantPanic: false,
		},
		{
			name:      "Set to center",
			align:     value.AlignItemsCenter,
			wantPanic: false,
		},
		{
			name:      "Invalid align panics",
			align:     value.AlignItems(99),
			wantPanic: true,
		},
	}
...
func TestFlexContainer_RemoveItem(t *testing.T) {
	...
	t.Run("Remove negative index panics", func(_ *testing.T) {
		...
		container := NewFlexContainer(value.FlexDirectionRow).AddItem(createNode("Item"))
...
func TestFlexContainer_IsHorizontal(t *testing.T) {
	row := NewFlexContainer(value.FlexDirectionRow)
	column := NewFlexContainer(value.FlexDirectionColumn)
...
func TestFlexContainer_String(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func() *FlexContainer {
				return NewFlexContainer(value.FlexDirectionRow)
			},
		...
		},
		{
			...
			setup: func() *FlexContainer {
				return NewFlexContainer(value.FlexDirectionColumn).
