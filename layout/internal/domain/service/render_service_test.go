...
func TestRender_WithBorder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			box := model2.NewBox(tt.content).WithBorder(true)
			output := rs.Render(box)
			assert.Equal(t, tt.expected, output,
...
func TestRender_WithPadding(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			box := model2.NewBox(tt.content).WithPadding(tt.padding)
			output := rs.Render(box)
			assert.Equal(t, tt.expected, output,
				"Output mismatch:\nGot:\n%s\n\nWant:\n%s", output, tt.expected)
		})
...
func TestRender_WithMargin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			box := model2.NewBox(tt.content).WithMargin(tt.margin)
			output := rs.Render(box)
			assert.Equal(t, tt.expected, output,
				"Output mismatch:\nGot:\n%s\n\nWant:\n%s", output, tt.expected)
		})
...
func TestRender_EdgeCases(t *testing.T) {
	...
	t.Run("zero padding", func(t *testing.T) {
		box := model2.NewBox("Test").WithPadding(value.NewSpacingZero())
		output := rs.Render(box)
		assert.Equal(t, "Test", output)
	})

	t.Run("zero margin", func(t *testing.T) {
		box := model2.NewBox("Test").WithMargin(value.NewSpacingZero())
		output := rs.Render(box)
		assert.Equal(t, "Test", output)
	})

	t.Run("border only (no padding/margin)", func(t *testing.T) {
		box := model2.NewBox("X").WithBorder(true)
		expected := strings.Join([]string{
			"┌───┐",
			"│ X │",
			"└───┘",
...
func TestRenderNode(t *testing.T) {
	...
	t.Run("single node", func(t *testing.T) {
		box := model2.NewBox("Hello").WithBorder(true)
	...
	t.Run("node with children (simplified, renders root only)", func(t *testing.T) {
		parentBox := model2.NewBox("Parent").WithBorder(true)
		childBox := model2.NewBox("Child")

		child := model2.NewNode(childBox)
		parent := model2.NewNode(parentBox).AddChild(child)
...
func TestRender_VisualVerification(t *testing.T) {
	...
	t.Run("print examples", func(t *testing.T) {
		examples := []struct {
			...
			{
				name: "Simple border",
				box:  model2.NewBox("Hello, World!").WithBorder(true),
			},
			{
				name: "With padding",
				box: model2.NewBox("Padded Box").
					WithPadding(value.NewSpacingAll(2)).
					WithBorder(true),
			},
			{
				name: "Full box model",
				box: model2.NewBox("Complete\nExample").
					WithPadding(value.NewSpacingAll(1)).
					WithBorder(true).
					WithMargin(value.NewSpacingAll(1)),
			},
			{
				name: "Dialog",
				box: model2.NewBox("Are you sure you want to quit?").
					WithPadding(value.NewSpacingVH(1, 2)).
					WithBorder(true).
