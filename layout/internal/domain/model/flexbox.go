...
type FlexContainer struct {
	direction      value2.FlexDirection  // Main axis direction
	justifyContent value2.JustifyContent // Main axis distribution
	alignItems     value2.AlignItems     // Cross axis alignment
	gap            int                   // Space between items (in cells)
	items          []*Node               // Child items (nodes wrapping boxes)
	size           value2.Size           // Container size constraints
}
...
func NewFlexContainer(direction value2.FlexDirection) *FlexContainer {
	...
	return &FlexContainer{
		direction:      direction,
		justifyContent: value2.JustifyContentStart,
		alignItems:     value2.AlignItemsStretch,
		gap:            0,
		items:          []*Node{},
		size:           value2.NewSizeUnconstrained(),
	}
}

// Direction returns the flex direction (row/column).
func (f *FlexContainer) Direction() value2.FlexDirection {
	return f.direction
}

// JustifyContent returns the justify content strategy.
func (f *FlexContainer) JustifyContent() value2.JustifyContent {
	return f.justifyContent
}

// AlignItems returns the align items strategy.
func (f *FlexContainer) AlignItems() value2.AlignItems {
	return f.alignItems
}
...
func (f *FlexContainer) Size() value2.Size {
	return f.size
}
...
func (f *FlexContainer) WithDirection(direction value2.FlexDirection) *FlexContainer {
	if !direction.Validate() {
		panic(fmt.Sprintf("flexbox: invalid direction %d", direction))
	}
...
func (f *FlexContainer) WithJustifyContent(justify value2.JustifyContent) *FlexContainer {
	if !justify.Validate() {
...
func (f *FlexContainer) WithAlignItems(align value2.AlignItems) *FlexContainer {
	if !align.Validate() {
...
func (f *FlexContainer) WithSize(size value2.Size) *FlexContainer {
	result := *f
	result.size = size
	return &result
}
