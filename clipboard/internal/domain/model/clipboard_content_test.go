...
func TestNewTextContent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.MIMEType() != value2.MIMETypePlainText {
				t.Errorf("expected plain text MIME type")
			}

			if result.Encoding() != value2.EncodingUTF8 {
...
func TestNewBinaryContent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.MIMEType() != value2.MIMETypeBinary {
				t.Errorf("expected binary MIME type")
			}

			if result.Encoding() != value2.EncodingBinary {
...
func TestClipboardContent_WithEncoding(t *testing.T) {
	...
	modified := original.WithEncoding(value2.EncodingBase64)

	if modified.Encoding() != value2.EncodingBase64 {
		t.Errorf("expected base64 encoding")
	}

	// Verify immutability
	if original.Encoding() != value2.EncodingUTF8 {
...
func TestClipboardContent_WithMIMEType(t *testing.T) {
	original := mustNewTextContent("Hello")
	modified := original.WithMIMEType(value2.MIMETypeHTML)

	if modified.MIMEType() != value2.MIMETypeHTML {
		t.Errorf("expected HTML MIME type")
	}

	// Verify immutability
	if original.MIMEType() != value2.MIMETypePlainText {
		t.Errorf("original should not be modified")
	}
}
