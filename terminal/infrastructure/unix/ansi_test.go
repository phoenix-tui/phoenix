...
func TestANSI_SetCursorStyle(t *testing.T) {
	tests := []struct {
		name  string
		style api.CursorStyle
		want  string
	}{
		{"block", api.CursorBlock, "\033[2 q"},
		{"underline", api.CursorUnderline, "\033[4 q"},
		{"bar", api.CursorBar, "\033[6 q"},
	}
...
func TestANSI_SetCursorStyle_Invalid(t *testing.T) {
	...
	err := term.SetCursorStyle(api.CursorStyle(99))
	if err == nil {
