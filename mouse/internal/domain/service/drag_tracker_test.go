...
func TestDragTracker_BasicDragFlow(t *testing.T) {
	...
	endPos := value2.NewPosition(15, 10)
	...
	if dragEvent.Type() != value2.EventDrag {
...
func TestDragTracker_ReleaseWithoutPress(t *testing.T) {
	...
	if start != value2.NewPosition(0, 0) {
		t.Errorf("Expected zero start position, got %v", start)
	}

	if end != value2.NewPosition(0, 0) {
...
func TestDragTracker_PreservesButtonAndModifiers(t *testing.T) {
	...
	modifiers := value2.ModifierShift | value2.ModifierCtrl
	...
	motionEvent := model.NewMouseEvent(
		value2.EventMotion,
		value2.ButtonLeft, // Different button (shouldn't matter)
		value2.NewPosition(15, 10),
		value2.ModifierNone, // Different modifiers (shouldn't matter)
	)
	dragEvent := tracker.ProcessMotion(motionEvent)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			motionEvent := model.NewMouseEvent(
				value2.EventMotion,
				value2.ButtonLeft,
				value2.NewPosition(10+tt.expected, 10),
				value2.ModifierNone,
			)
			tracker.ProcessMotion(motionEvent)
			...
			motionEvent2 := model.NewMouseEvent(
				value2.EventMotion,
				value2.ButtonLeft,
				value2.NewPosition(10+tt.expected-1, 10),
				value2.ModifierNone,
			)
...
func TestDragTracker_DistanceCalculation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			pos:              value2.NewPosition(13, 10),
			expectedDistance: 3,
		},
		{
			name:             "4 cells vertical",
			pos:              value2.NewPosition(10, 14),
			expectedDistance: 4,
		},
		{
			name:             "Manhattan distance 3+4=7",
			pos:              value2.NewPosition(13, 14),
...
func TestDragTracker_ContinuousMotion(t *testing.T) {
	...
	positions := []value2.Position{
		value2.NewPosition(11, 10), // 1 cell (no drag)
		value2.NewPosition(12, 10), // 2 cells (drag starts)
		value2.NewPosition(13, 10), // 3 cells (drag continues)
		value2.NewPosition(14, 10), // 4 cells (drag continues)
		value2.NewPosition(15, 10), // 5 cells (drag continues)
	}
...
func TestDragTracker_OnlyPressStartsDrag(t *testing.T) {
	...
	nonPressEvents := []value2.EventType{
		value2.EventRelease,
		value2.EventMotion,
		value2.EventClick,
...
func TestDragTracker_UpdateWithoutStart(t *testing.T) {
	...
	state.Update(value2.NewPosition(100, 100))
