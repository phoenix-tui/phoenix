...
func TestProgram_ExecProcess_APIWrapper(t *testing.T) {
	...
	m := TestModel{value: 0}
	p := api.New(m, api.WithTerminal[TestModel](mockTerm))
...
func TestProgram_ExecProcess_API_WithAltScreen(t *testing.T) {
	mockTerm := phoenixtesting.NewMockTerminal()
	m := TestModel{value: 0}
	p := api.New(m, api.WithTerminal[TestModel](mockTerm), api.WithAltScreen[TestModel]())
...
func TestAPI_ExecProcess_Integration(t *testing.T) {
	...
	p := api.New(
		m,
		api.WithTerminal[TestModel](mockTerm),
