...
func (m TestModel) Init() model2.Cmd {
...
func (m TestModel) Update(msg model2.Msg) (model2.Model[TestModel], model2.Cmd) {
	m.updateCount++
	...
	case model2.KeyMsg:
		...
		if msg.String() == "q" {
			return m, func() model2.Msg { return model2.QuitMsg{} }
		}
	case model2.QuitMsg:
		m.lastMsg = "quit"
	}
...
func TestProgram_EventLoop_Init(t *testing.T) {
	...
	// Send quit
	if err := p.Send(model2.QuitMsg{}); err != nil {
		t.Fatal(err)
	}
	p.Stop()
...
func TestProgram_EventLoop_Update(t *testing.T) {
	...
	// Send messages
	if err := p.Send(model2.KeyMsg{Type: model2.KeyRune, Rune: '+'}); err != nil {
		t.Fatal(err)
	}
	time.Sleep(50 * time.Millisecond)
...
func TestProgram_EventLoop_BatchMsg(t *testing.T) {
	...
	batchMsg := model2.BatchMsg{
		Messages: []model2.Msg{
			model2.KeyMsg{Type: model2.KeyRune, Rune: '+'},
			model2.KeyMsg{Type: model2.KeyRune, Rune: '+'},
			model2.KeyMsg{Type: model2.KeyRune, Rune: '+'},
		},
	}
...
func TestProgram_Send(t *testing.T) {
	...
	err := p.Send(model2.KeyMsg{Type: model2.KeyEnter})
	if err == nil {
