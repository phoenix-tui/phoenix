...
func NewTerminal() *Terminal {
	...
	return &Terminal{domain: model.NewTerminal(caps)}
}
...
func NewTerminalWithCapabilities(caps *Capabilities) *Terminal {
	...
	return &Terminal{domain: model.NewTerminal(caps.domain)}
}
...
func (t *Terminal) WithSize(size Size) *Terminal {
	domainSize := value.NewSize(size.Width, size.Height)
...
func (p Position) Add(deltaRow, deltaCol int) Position {
	domainPos := value.NewPosition(p.Row, p.Col)
...
func NewCellAuto(content string) Cell {
	...
	unicodeService := service.NewUnicodeService()
	width := unicodeService.StringWidth(content)

	// Create Cell with calculated width
	domainCell := value.NewCell(content, width)
	return Cell{
...
func NewRawMode(originalState interface{}) (*RawMode, error) {
	domain, err := model.NewRawMode(originalState)
	if err != nil {
		return nil, err
	}
...
func NewCapabilities(ansi bool, colorDepth ColorDepth, mouse bool, altScreen bool, cursor bool) *Capabilities {
	domainColorDepth := value.ColorDepth(colorDepth)
	domain := value.NewCapabilities(ansi, domainColorDepth, mouse, altScreen, cursor)
