...
func TestNewTerminal(t *testing.T) {
	tests := []struct {
		name     string
		caps     *value2.Capabilities
		wantANSI bool
	}{
		{
			name:     "with full capabilities",
			caps:     value2.NewCapabilities(true, value2.ColorDepthTrueColor, true, true, true),
			wantANSI: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			term := model2.NewTerminal(tt.caps)
...
func TestTerminal_WithSize(t *testing.T) {
	caps := value2.NewCapabilities(true, value2.ColorDepth256, true, true, true)
	term := model2.NewTerminal(caps)
	...
	tests := []struct {
		...
		{
			name:    "different size",
			newSize: value2.NewSize(90, 30),
		},
		{
			name:    "large terminal",
			newSize: value2.NewSize(200, 60),
		},
		{
			name:    "small terminal",
			newSize: value2.NewSize(40, 12),
		},
	}
...
func TestTerminal_WithRawMode(t *testing.T) {
	...
	rawMode, err := model2.NewRawMode("original-state")
	if err != nil {
...
func TestTerminal_WithoutRawMode(t *testing.T) {
	...
	rawMode, _ := model2.NewRawMode("original-state")
	rawMode = rawMode.Enable()
	termRaw := term.WithRawMode(rawMode)
...
func TestTerminal_ConvenienceMethods(t *testing.T) {
	tests := []struct {
		...
		wantANSI       bool
		wantColor      bool
		wantColorDepth value2.ColorDepth
	}{
		{
			...
			wantANSI:       true,
			wantColor:      true,
			wantColorDepth: value2.ColorDepthTrueColor,
		},
		{
			...
			wantANSI:       false,
			wantColor:      false,
			wantColorDepth: value2.ColorDepthNone,
		},
		{
			name:           "ANSI but no color",
			caps:           value2.NewCapabilities(true, value2.ColorDepthNone, false, false, false),
			wantANSI:       true,
			wantColor:      false,
			wantColorDepth: value2.ColorDepthNone,
		},
...
func TestTerminal_Immutability(t *testing.T) {
	...
	term2 := term1.WithSize(value2.NewSize(100, 50))
	rawMode, _ := model2.NewRawMode("state")
