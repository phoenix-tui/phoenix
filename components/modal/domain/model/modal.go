...
type Modal struct {
	...
	size          *value.Size     // Modal size (width, height)
	position      *value.Position // Position (center or custom x, y)
...
func (m *Modal) WithSize(width, height int) *Modal {
	return &Modal{
		...
		content:       m.content,
		buttons:       m.buttons,
		size:          value.NewSize(width, height),
		position:      m.position,
		focusedButton: m.focusedButton,
		visible:       m.visible,
		dimBackground: m.dimBackground,
...
func (m *Modal) WithPosition(position *value.Position) *Modal {
	return &Modal{
		title:         m.title,
		content:       m.content,
		buttons:       m.buttons,
...
func (m *Modal) Size() *value.Size {
	return m.size
}

// Position returns the modal position.
func (m *Modal) Position() *value.Position {
	return m.position
}
