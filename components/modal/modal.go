...
import (
	...
	model2 "github.com/phoenix-tui/phoenix/components/modal/internal/domain/model"
...
func New(content string) *Modal {
	return &Modal{
		domain:         model2.NewModal(content),
...
func NewWithTitle(title, content string) *Modal {
	return &Modal{
		domain:         model2.NewModalWithTitle(title, content),
		layoutService:  service.NewLayoutService(),
		keyBindings:    infrastructure.DefaultKeyBindings(),
		terminalWidth:  80,
		terminalHeight: 24,
...
func (m *Modal) Buttons(buttons []Button) *Modal {
	domainButtons := make([]*model2.Button, len(buttons))
	for i, btn := range buttons {
		domainButtons[i] = model2.NewButton(btn.Label, btn.Key, btn.Action)
	}
...
func (m *Modal) renderButtons(availableWidth int) string {
	...
	parts := make([]string, 0, len(m.domain.Buttons())+3) // Pre-allocate: buttons + title + content + spacing
