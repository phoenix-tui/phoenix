...
import (
	...
	"github.com/phoenix-tui/phoenix/components/table/domain/service"
	"github.com/phoenix-tui/phoenix/components/table/domain/value"
	"github.com/phoenix-tui/phoenix/components/table/infrastructure"
...
type Column struct {
	...
	Alignment value.Alignment          // Cell alignment (default: left)
...
func New(columns []Column) *Table {
	domainCols := make([]*model.Column, len(columns))
	for i, col := range columns {
		domainCol := model.NewColumn(col.Key, col.Title, col.Width)
		if col.Alignment != value.AlignmentLeft {
	...
	return &Table{
		domain:      model.NewTable(domainCols),
