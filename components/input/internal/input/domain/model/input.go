...
import (
	service2 "github.com/phoenix-tui/phoenix/components/input/internal/input/domain/service"
	value2 "github.com/phoenix-tui/phoenix/components/input/internal/input/domain/value"
)
...
type TextInput struct {
	...
	cursor         *value2.Cursor          // Cursor position (grapheme-aware)
	selection      *value2.Selection       // Selection range (nil if no selection)
	validator      service2.ValidationFunc // Validation hook (nil if no validation)
	...
	cursorMovement *service2.CursorMovementService
	validationSvc  *service2.ValidationService
}
...
func New(width int) *TextInput {
	...
	return &TextInput{
		...
		cursorMovement: service2.NewCursorMovementService(),
		validationSvc:  service2.NewValidationService(),
	}
}
...
func (t *TextInput) Selection() *value2.Selection {
	if t.selection == nil {
		return nil
	}
...
func (t TextInput) SetContent(content string, cursorPos int) TextInput {
	...
	t.cursor = value2.NewCursor(cursorPos)
...
func (t TextInput) WithCursor(pos int) TextInput {
	...
	t.cursor = value2.NewCursor(pos).MoveTo(pos, maxOffset)
	return t
}
...
func (t TextInput) WithSelection(start, end int) TextInput {
	maxOffset := t.cursorMovement.GraphemeCount(t.content)
	t.selection = value2.NewSelection(start, end).Clamp(maxOffset)

	// Move cursor to end of selection.
	t.cursor = value2.NewCursor(t.selection.End())

	return t
}

// WithValidator sets the validation function (immutable).
func (t TextInput) WithValidator(validator service2.ValidationFunc) TextInput {
...
func (t TextInput) MoveLeft() TextInput {
	...
	t.cursor = value2.NewCursor(newPos)
...
func (t TextInput) MoveEnd() TextInput {
	maxOffset := t.cursorMovement.GraphemeCount(t.content)
	t.cursor = value2.NewCursor(maxOffset)
	t.selection = nil // Clear selection on cursor movement
	return t
}
...
func (t TextInput) SelectAll() TextInput {
	maxOffset := t.cursorMovement.GraphemeCount(t.content)
	t.selection = value2.NewSelection(0, maxOffset)
	t.cursor = value2.NewCursor(maxOffset)
	return t
}
...
func (t TextInput) deleteSelection() TextInput {
	...
	t.cursor = value2.NewCursor(t.selection.Start())
	t.selection = nil
