...
import (
	...
	"github.com/phoenix-tui/phoenix/components/input/internal/input/domain/service"
	"github.com/phoenix-tui/phoenix/components/input/internal/input/infrastructure"
...
func (i Input) Validator(fn ValidationFunc) Input {
	// Convert public ValidationFunc to internal service.ValidationFunc
	internalValidator := service.ValidationFunc(fn)
	i.domain = i.domain.WithValidator(internalValidator)
	return i
}
...
func (i Input) renderContent() string {
	...
	// Note: Uses byte offsets for simplicity - works for ASCII and most Unicode.
	// For complex grapheme clusters (emoji with modifiers), phoenix/core UnicodeService
	// should be used, but that's a future optimization.
...
func Chain(validators ...ValidationFunc) ValidationFunc {
	// Convert public validators to internal
	internalValidators := make([]service.ValidationFunc, len(validators))
	for i, v := range validators {
		internalValidators[i] = service.ValidationFunc(v)
	}
	internal := service.Chain(internalValidators...)
	return ValidationFunc(internal)
}
