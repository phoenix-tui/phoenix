...
//	l := list.New(files, files, list.SelectionModeSingle).Height(10)
//	p := tea.New(l)
//	p.Run()
//
...
//
//	l := list.New(people, getLabels(people), list.SelectionModeSingle).
...
func New(values []interface{}, labels []string, mode SelectionMode) *List {
	...
	// Convert public SelectionMode to internal
	internalMode := value.SelectionMode(mode)

	return &List{
		domain:     model.NewListWithItems(items, internalMode),
...
func NewSingleSelect(values []interface{}, labels []string) *List {
	return New(values, labels, SelectionModeSingle)
}
...
func NewMultiSelect(values []interface{}, labels []string) *List {
	return New(values, labels, SelectionModeMulti)
}
...
//
//nolint:gocyclo,cyclop // Keyboard handling is complex by nature: processes multiple navigation/selection keys
