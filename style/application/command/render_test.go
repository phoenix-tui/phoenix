...
func TestRenderCommand_Execute_BothColors(t *testing.T) {
	cmd := newRenderCommand()
	fg := value.RGB(255, 255, 255) // White
	bg := value.RGB(0, 0, 0)       // Black
	style := model.NewStyle().
		Foreground(fg).
		Background(bg)
...
func TestRenderCommand_Execute_MarginOnly(t *testing.T) {
	cmd := newRenderCommand()
	margin := value.NewMargin(1, 2, 1, 2)
...
func TestRenderCommand_Execute_BorderOnly(t *testing.T) {
	cmd := newRenderCommand()
	style := model.NewStyle().Border(value.NormalBorder)
	content := "Hello"
...
func TestRenderCommand_Execute_SelectiveBorders(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			style:     model.NewStyle().Border(value.NormalBorder).BorderTop(true).BorderBottom(true),
			topBorder: true,
			botBorder: true,
			leftSide:  false,
			rightSide: false,
		},
		{
			...
			style:     model.NewStyle().Border(value.NormalBorder).BorderTop(false).BorderBottom(false).BorderLeft(true).BorderRight(true),
			topBorder: false,
			botBorder: false,
			leftSide:  true,
			rightSide: true,
		...
		{
			name:      "top only",
			style:     model.NewStyle().Border(value.NormalBorder).BorderTop(true),
			topBorder: true,
			botBorder: false,
			leftSide:  false,
			rightSide: false,
...
func TestRenderCommand_Execute_WidthConstraint(t *testing.T) {
	...
	unicodeService := coreService.NewUnicodeService()
	actualWidth := unicodeService.StringWidth(strings.TrimRight(output, "\n"))
	if actualWidth != 10 {
...
func TestRenderCommand_Execute_Alignment(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "left aligned",
			alignment: value.NewAlignment(value.AlignLeft, value.AlignTop),
			width:     20,
			content:   "Hello",
		},
		{
		...
		{
			name:      "right aligned",
			alignment: value.NewAlignment(value.AlignRight, value.AlignBottom),
			width:     20,
			content:   "Hello",
		},
	}
...
func TestRenderCommand_Execute_ColorAdaptation(t *testing.T) {
	tests := []struct {
		name       string
		capability value.TerminalCapability
	}{
...
func TestRenderCommand_Execute_BorderColor(t *testing.T) {
	cmd := newRenderCommand()
	borderColor := value.RGB(255, 0, 0) // Red
	style := model.NewStyle().
		Border(value.NormalBorder).
