...
// Example:.
//
//	cmd := NewRenderCommand(colorAdapter, spacingCalc, textAligner, ansiGen).
//	style := model.NewStyle().Foreground(value.RGB(255, 0, 0)).Bold(true).
//	output, err := cmd.Execute(style, "Hello, World!").
type RenderCommand struct {
	colorAdapter      service2.ColorAdapter
	spacingCalculator service2.SpacingCalculator
	textAligner       service2.TextAligner
...
func NewRenderCommand(
	...
	return &RenderCommand{
		...
		borderRenderer:    NewBorderRenderer(ansiGenerator),
	}
}
