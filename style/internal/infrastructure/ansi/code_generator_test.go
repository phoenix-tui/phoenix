...
// TestReverse tests reverse video code generation.
...
// TestReverseOff tests reverse-off code generation.
func TestReverseOff(t *testing.T) {
	gen := NewANSICodeGenerator()
	want := "\x1b[27m"
	got := gen.ReverseOff()

	if got != want {
		t.Errorf("ReverseOff() = %q, want %q", got, want)
	}
}
