...
func TestCalculateTotalWidth(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			contentWidth: 10,
			padding:      value2.UniformPadding(0),
			margin:       value2.UniformMargin(0),
			want:         10,
		},
		{
		...
		},
		{
			...
			contentWidth: 10,
			padding:      value2.NewPadding(0, 2, 0, 3), // top, right, bottom, left
			margin:       value2.UniformMargin(0),
			want:         15, // 10 + 2 + 3
		},
		{
			...
			contentWidth: 10,
			padding:      value2.UniformPadding(0),
			margin:       value2.NewMargin(0, 4, 0, 5), // top, right, bottom, left
			want:         19,                           // 10 + 4 + 5
		},
	}
...
func TestCalculateTotalHeight(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:          "asymmetric padding",
			contentHeight: 5,
			padding:       value2.NewPadding(2, 0, 3, 0), // top, right, bottom, left
			margin:        value2.UniformMargin(0),
			want:          10, // 5 + 2 + 3
		},
	}
...
func TestApplyPadding(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			content: "Hello",
			padding: value2.NewPadding(0, 0, 0, 2),
			want:    "  Hello",
		},
		{
			name:    "right padding only",
			content: "Hello",
			padding: value2.NewPadding(0, 2, 0, 0),
			want:    "Hello  ",
		},
		{
			name:    "top padding only",
			content: "Hello",
			padding: value2.NewPadding(2, 0, 0, 0),
			want: strings.Join([]string{
				"     ", // empty line (width 5)
				"     ", // empty line
				"Hello",
...
func TestApplyMargin(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			content: "Hello",
			margin:  value2.NewMargin(0, 0, 0, 2),
			want:    "  Hello",
		},
		{
			name:    "multi-line content",
...
func TestApplyBoth(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			content: "A",
			padding: value2.NewPadding(0, 1, 0, 1), // horizontal padding only
			margin:  value2.NewMargin(0, 2, 0, 2),  // horizontal margin only
...
func TestApplyPaddingWithUnicode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:    "emoji content",
			content: "👋",
			padding: value2.NewPadding(0, 1, 0, 1),
			check: func(t *testing.T, got string) {
				want := " 👋 "
				if got != want {
...
func TestSpacingCalculatorEdgeCases(t *testing.T) {
	...
	t.Run("zero width content", func(t *testing.T) {
		width := calc.CalculateTotalWidth(0, value2.UniformPadding(2), value2.UniformMargin(3))
		want := 10 // 0 + 2*2 + 3*2
		if width != want {
	...
	t.Run("single space content", func(t *testing.T) {
		got := calc.ApplyPadding(" ", value2.UniformPadding(1))
