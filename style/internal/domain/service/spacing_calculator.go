...
type SpacingCalculator interface {
	...
	CalculateTotalWidth(contentWidth int, padding value2.Padding, margin value2.Margin) int

	// CalculateTotalHeight calculates total height including content, padding, and margin.
	CalculateTotalHeight(contentHeight int, padding value2.Padding, margin value2.Margin) int

	// ApplyPadding adds padding around content.
	// Padding adds space INSIDE the content box.
	ApplyPadding(content string, padding value2.Padding) string

	// ApplyMargin adds margin around content.
	// Margin adds space OUTSIDE the content box.
	ApplyMargin(content string, margin value2.Margin) string

	// ApplyBoth applies both padding and margin (padding first, then margin).
	ApplyBoth(content string, padding value2.Padding, margin value2.Margin) string
}
...
func (sc *DefaultSpacingCalculator) CalculateTotalWidth(contentWidth int, padding value2.Padding, margin value2.Margin) int {
...
func (sc *DefaultSpacingCalculator) CalculateTotalHeight(contentHeight int, padding value2.Padding, margin value2.Margin) int {
...
func (sc *DefaultSpacingCalculator) ApplyPadding(content string, padding value2.Padding) string {
	if content == "" {
		return ""
	}
	...
	if len(paddedLines) > 0 {
		emptyLineWidth = core.StringWidth(paddedLines[0])
	}
...
func (sc *DefaultSpacingCalculator) ApplyMargin(content string, margin value2.Margin) string {
	if content == "" {
		return ""
	}
	...
	if len(marginedLines) > 0 {
		emptyLineWidth = core.StringWidth(marginedLines[0])
	}
	emptyLine := strings.Repeat(" ", emptyLineWidth)
...
func (sc *DefaultSpacingCalculator) ApplyBoth(content string, padding value2.Padding, margin value2.Margin) string {
