...
func TestNewStyle(t *testing.T) {
	...
	assert.Equal(t, value.TrueColor, s.GetTerminalCapability(), "default terminal capability should be TrueColor")
}
...
func TestStyle_AlignHorizontal(t *testing.T) {
	s := NewStyle().AlignHorizontal(value.AlignCenter)

	a, hasAlign := s.GetAlignment()
	assert.True(t, hasAlign, "alignment should be set")
	assert.Equal(t, value.AlignCenter, a.Horizontal(), "horizontal alignment should be center")
	assert.Equal(t, value.AlignTop, a.Vertical(), "vertical alignment should default to top")
}

// TestStyle_AlignVertical tests setting vertical alignment.
func TestStyle_AlignVertical(t *testing.T) {
	s := NewStyle().AlignVertical(value.AlignMiddle)

	a, hasAlign := s.GetAlignment()
	assert.True(t, hasAlign, "alignment should be set")
	assert.Equal(t, value.AlignLeft, a.Horizontal(), "horizontal alignment should default to left")
	assert.Equal(t, value.AlignMiddle, a.Vertical(), "vertical alignment should be middle")
}
...
func TestStyle_TerminalCapability(t *testing.T) {
	s := NewStyle().TerminalCapability(value.ANSI256)
	assert.Equal(t, value.ANSI256, s.GetTerminalCapability(), "terminal capability should be ANSI256")

	s = NewStyle().TerminalCapability(value.ANSI16)
	assert.Equal(t, value.ANSI16, s.GetTerminalCapability(), "terminal capability should be ANSI16")
}

// TestStyle_Immutability tests that setter methods return new instances.
func TestStyle_Immutability(t *testing.T) {
	original := NewStyle()
	modified := original.Foreground(value.RGB(255, 0, 0))
