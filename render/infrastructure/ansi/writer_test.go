...
func TestWriter_WriteCellAt(t *testing.T) {
	...
	cell := model.NewCell('X', value.NewStyle())
...
func TestWriter_WriteString(t *testing.T) {
	...
	style := value.NewStyleWithFg(value.ColorBlue)
...
func TestWriter_CurrentStyle(t *testing.T) {
	...
	// After setting style
	newStyle := value.NewStyleWithFg(value.ColorRed)
...
func TestWriter_BufferedSize(t *testing.T) {
	...
	// After writing
	err := w.WriteString("test", value.NewStyle())
	require.NoError(t, err)
