...
func (b *Buffer) Get(pos value2.Position) Cell {
	x, y := pos.X(), pos.Y()
...
func (b *Buffer) Set(pos value2.Position, cell Cell) {
	x, y := pos.X(), pos.Y()
	if x < 0 || x >= b.width || y < 0 || y >= b.height {
		return
	}
...
func (b *Buffer) Fill(char rune, style value2.Style) {
...
func (b *Buffer) SetString(pos value2.Position, text string, style value2.Style) int {
	x, y := pos.X(), pos.Y()
	...
	for text != "" {
		...
		b.Set(value2.NewPosition(x, y), cell)
...
func (b *Buffer) SetLine(y int, text string, style value2.Style) {
	if y < 0 || y >= b.height {
		return
	}

	// Write text.
	pos := value2.NewPosition(0, y)
	written := b.SetString(pos, text, style)

	// Clear rest of line.
	for x := written; x < b.width; x++ {
		b.Set(value2.NewPosition(x, y), NewEmptyCell())
	}
}
...
func (b *Buffer) SetCells(positions []value2.Position, cells []Cell) {
