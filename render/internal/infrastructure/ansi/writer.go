...
func (w *Writer) SetStyle(style value2.Style) error {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *Writer) WriteCellAt(pos value2.Position, cell model.Cell) error {
...
func (w *Writer) WriteString(text string, style value2.Style) error {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *Writer) WriteStringAt(pos value2.Position, text string, style value2.Style) error {
	if err := w.MoveCursor(pos.X(), pos.Y()); err != nil {
		return err
	}
...
func (w *Writer) Reset() error {
	...
	w.currentStyle = value2.NewStyle()
	return nil
}
...
func (w *Writer) CurrentStyle() value2.Style {
	w.mu.Lock()
	defer w.mu.Unlock()
