...
import (
	...
	service2 "github.com/phoenix-tui/phoenix/render/internal/domain/service"
	"github.com/phoenix-tui/phoenix/render/internal/domain/value"
	"github.com/phoenix-tui/phoenix/render/internal/infrastructure/ansi"
)
...
type Renderer struct {
	diffService     *service2.DiffService
	optimizeService *service2.OptimizeService
	writer          *ansi.Writer
...
func NewRenderer(width, height int, output io.Writer) *Renderer {
	r := &Renderer{
		diffService:     service2.NewDiffService(),
		optimizeService: service2.NewOptimizeService(),
...
func (r *Renderer) applyOperations(ops []service2.DiffOp) error {
	for _, op := range ops {
		switch op.Type {
		case service2.OpTypeSet:
		...
		case service2.OpTypeMoveCursor:
			if err := r.writer.MoveCursor(op.Position.X(), op.Position.Y()); err != nil {
				return err
			}
		}
